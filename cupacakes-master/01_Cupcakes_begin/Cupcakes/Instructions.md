...
#### Task 3: Set up Entity Framework Core to use SQLServer
...
4. Click **Microsoft.EntityFrameworkCore.SQLServer**, select the last version, and then click **Install**.
...
13. In the **Startup.cs** code window, in the **ConfigureServices** method, place the cursor after the **{** (opening braces) sign, press Enter, type the following code, and then press Enter.
  ```cs
      services.AddDbContext<CupcakeContext>(options =>
              options.UseSqlServer(@"Server=(localdb)\mssqllocaldb;Database=cupcakeDb;Trusted_Connection=True;");
```
...
9. In the **Cupcake Picture** box, import an image from **Labfiles\Image\strawberry-cupcake.jpg**, and then click **Submit**.
...
6. Replace the selected code with the hardcoded connectionstring with the following code:
  ```cs
      services.AddDbContext<CupcakeContext>(options =>
...
2. In **cmder cli** , type the following command, and then press Enter.
  ```cs
     dotnet ef migrations add InitialCreate --output-dir Migrations --project {ProjectName}
```
>**Note**: In **Solution Explorer**, verify if a new folder named **Migrations** is created with multiple files.

3. In the **cmder cli** , type the following command but replace the {} with appropriate values and then press Enter.
  ```cs
        dotnet ef migrations script {lastgeneratedscript} --project {ProjectName}
 
```
4. In the **Cupcakes - Microsoft Visual Studio** window, on the **VIEW** menu, click **SQL Server Object Explorer**.

5. In **SQL Server Object Explorer**, expand **(localdb)\MSSQLLocalDB**, expand **Databases**, and then expand **cupcakesDb**.

>**Note**: View the **cupcakesDb** tables.
...
22. In the **cmder cli** , type the following command, and then press Enter.
  ```cs
      dotnet ef migrations add AddCupcakeCaloricValue --output-folder Migrations
```
>**Note**: In **Solution Explorer**, under **Migrations**, verify if a new file is created.

24. In **cmder cli** , type the following command and replace the {} placeholders with the right values, and then press Enter.
  ```cs
     dotnet ef migrations script {Lastworking migration} --project {ProjectName}
```
